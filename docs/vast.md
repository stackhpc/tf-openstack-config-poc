...
> [!Caution]
> `vast_password` ends up in the state.

## VAST Vippools

To create a VAST vippool,
...
## Connecting VAST to OpenStack

To connect the VAST resource to OpenStack, `sharetypes` and `sharetypes_access`
resources need to be used. The `sharetypes` resource is required to have
`vast_vippool_name` defined in the `extra_specs` block to connect VAST. Further
documentation can be found in [docs/shared_filesystem.md](./shared_filesystem.md).
